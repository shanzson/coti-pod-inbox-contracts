...
import "@coti-io/coti-contracts/contracts/pod/fee/IInboxFeeManager.sol";
...
import "@coti-io/coti-contracts/contracts/pod/token/perc20/cotiside/IPodErc20CotiSide.sol";
import "@coti-io/coti-contracts/contracts/pod/token/erc7984/PodErc7984Mixin.sol";
...
contract PodERC20Fixed is IPodERC20, InboxUser, PodErc7984Mixin, ReentrancyGuard, Ownable {
    ...
    /// FIX #14: a request id that already has a record cannot be reused.
    error RequestIdAlreadyUsed(bytes32 requestId, IPodERC20.RequestStatus status);
    ...
    function syncBalances(address[] calldata accounts, uint256 callbackFeeLocalWei) external payable returns (bytes32 requestId) {
        ...
        requestId = _sendPodTwoWay(
            ...
            callbackFeeLocalWei,
            mpcMethodCall,
            PodERC20Fixed.syncBalancesCallback.selector,
            PodERC20Fixed.syncBalancesError.selector
        );
    ...
    function invalidatePendingRequest(bytes32 requestId) external {
        // FIX #3: the Ownable owner (the factory, via an admin-gated forwarder) may also invalidate, so a
        //         retired portal's admin refund is not bricked once the minter has been rotated away.
        if (msg.sender != owner()) {
            _checkMinter();
        }
    ...
    function _setRequestStatus(bytes32 requestId, IPodERC20.RequestStatus status) internal {
        ...
        // FIX #14: a request id is single-use. A fresh send must never overwrite a live or terminal record
        //          (ids are unique only per inbox instance; a re-pointed inbox restarts its nonce).
        if (status == IPodERC20.RequestStatus.Pending && current != IPodERC20.RequestStatus.None) {
            revert RequestIdAlreadyUsed(requestId, current);
        }
    ...
    function _approve(address owner, address spender, itUint256 calldata value, uint256 totalValueWei, uint256 callbackFeeLocalWei) internal returns (bytes32 requestId) {
        ...
        requestId = _sendPodTwoWay(
            ...
            callbackFeeLocalWei,
            mpcMethodCall,
            PodERC20Fixed.approveCallback.selector,
            PodERC20Fixed.approveError.selector
        );
        _setRequestStatus(requestId, IPodERC20.RequestStatus.Pending);
    ...
    function _burn(address from, itUint256 calldata value, uint256 totalValueWei, uint256 callbackFeeLocalWei) internal returns (bytes32 requestId) {
        ...
        requestId = _sendPodTwoWay(
            ...
            callbackFeeLocalWei,
            mpcMethodCall,
            PodERC20Fixed.transferCallback.selector,
            PodERC20Fixed.transferError.selector
        );
